module sui_file_explorer::file_system {
    ...
    struct File has key, store {
        ...
        name: String,
        parent_id: ID,
        blob_id: ID  // Reference to Walrus blob
    }
    ...
    public entry fun create_file(
        ...
        let blob_object_id = object::id_from_bytes(blob_id);
        let new_file = File {
            id: object::new(ctx),
            name: file_name,
    ...
    // Get file details
    public fun get_file_details(file: &File): (String, ID) {
        (file.name, file.blob_id)
    }
